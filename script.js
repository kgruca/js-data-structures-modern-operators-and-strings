...
const getAirlineCode = str => str.slice(0, 3).toUpperCase();
...
for (const flight of flights.split('+')) {
  ...
  const output = `${type.startsWith('_Delayed') ? '🔴' : ''}${type.replaceAll('_', ' ')} from ${getAirlineCode(from)} to ${getAirlineCode(to)} (${time.replace(':', 'h')})`.padStart(45);
  console.log(output);
}
// pass 1 logs _Delayed_Departure fao93766109 txl2133758440 (11:25)
